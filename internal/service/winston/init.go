...
func (s *WinstonServiceImpl) Init() error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	listenHost, err := (&winstonConfigListenHostImpl{}).Get(s.actionHandler)
	if err != nil {
		return err
	}
	s.listenHost = listenHost.(string)

	listenPort, err := (&winstonConfigListenPortImpl{}).Get(s.actionHandler)
	if err != nil {
		return err
	}
	s.listenPort = listenPort.(int)

	bufferSize, err := (&winstonConfigBufferSizeImpl{}).Get(s.actionHandler)
	if err != nil {
		return err
	}
	s.bufferSize = bufferSize.(int)
	s.ringBuffer = ringbuf.New[winstonRingBuffer](s.bufferSize)
